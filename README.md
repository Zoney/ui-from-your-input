...
Default backend is [Vercel AI Gateway](https://vercel.com/docs/ai-gateway) — one key, many providers and models behind an OpenAI-compatible endpoint. Default model is `mistral/ministral-3b` (tiny + cheap); swap in any [listed model](https://vercel.com/ai-gateway/models) via env. ~200 lines of Go, stdlib only.
...
| `INFERENCE_MODEL` | `mistral/ministral-3b` | Model id passed in the request body. Any AI Gateway model works, e.g. `openai/gpt-5.4-mini`, `anthropic/claude-haiku-4.5`, `google/gemini-2.5-flash`. |
